...
export function IntegrationsManager() {
  ...
  const [saveCnpjaError, setSaveCnpjaError] = useState<string | null>(null);
  const [cnpjaTestResult, setCnpjaTestResult] = useState<{ ok: boolean; message: string } | null>(null);
  ...
  async function saveCnpja(e: React.FormEvent) {
    e.preventDefault();
    setSavingCnpja(true);
    setCnpjaTestResult(null);
    setSaveCnpjaError(null);
    try {
      const res = await fetch("/api/settings/integrations", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider: "cnpja", apiKey: cnpjaApiKey }),
      });
      const json = await res.json().catch(() => ({}));
      if (res.ok) {
        setCnpjaApiKey("");
        ...
        setSaveCnpjaError(json.error ?? "Erro ao salvar a chave.");
      }
    } catch {
      setSaveCnpjaError("Erro de rede ao salvar a chave.");
    } finally {
      setSavingCnpja(false);
    }
  }

  async function testCnpjaConn() {
    setCnpjaTestResult(null);
    const res = await fetch("/api/settings/integrations/test-cnpja", { method: "POST" });
    const json = await res.json();
    setCnpjaTestResult({ ok: res.ok, message: res.ok ? `Conexão OK (${json.sample ?? "sem nome"}).` : json.error ?? "Falha ao conectar." });
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-slate-200">CNPJá · Dados da Receita Federal</h2>
            <p className="mt-0.5 text-xs text-slate-500">
              Chave atual: {status?.cnpja.apiKeyMasked ?? "não configurada"} · última consulta:{" "}
              {status?.cnpja.lastSyncAt ? new Date(status.cnpja.lastSyncAt).toLocaleString("pt-BR") : "nunca"}{" "}
              <StatusPill status={status?.cnpja.lastStatus ?? null} />
            </p>
            {status?.cnpja.lastError && <p className="mt-1 text-xs text-red-400">{status.cnpja.lastError}</p>}
          </div>
        </div>

        <form onSubmit={saveCnpja} className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <input
            required
            placeholder="Chave da API (API Key)"
            value={cnpjaApiKey}
          ...
          <button
            type="submit"
            disabled={savingCnpja}
            className="rounded-full bg-brand px-4 py-2 text-sm font-bold text-brand-ink transition hover:bg-brand-hover disabled:opacity-60"
          >
            {savingCnpja ? "Salvando..." : "Salvar chave"}
          </button>
          <button
            type="button"
            onClick={testCnpjaConn}
            className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-medium text-slate-300 hover:border-slate-500"
          >
            Testar conexão
          </button>
        </form>
        {saveCnpjaError && <p className="mt-3 text-xs text-red-400">{saveCnpjaError}</p>}
        {cnpjaTestResult && (
          <p className={`mt-3 text-xs ${cnpjaTestResult.ok ? "text-emerald-400" : "text-red-400"}`}>
            {cnpjaTestResult.message}
          </p>
        )}
        <p className="mt-3 text-xs text-slate-500">
          Consulta CNPJ com dados da Receita (situação cadastral + sócios). Não busca empresa a partir de nome de
          pessoa — na tela <span className="text-slate-300">Prospecção</span>, informe o CNPJ de uma empresa já
          encontrada pra consultar.
        </p>
      </section>
